...
experiment=conv3d_recreating_rwd_model_f12_recon_nonorm.yaml
...
ckpt_path=/home/krillman/Documents/eg3dredo/supp_plus_code/eg3d_rlhf_code/reward_model_training/reward_model_framework/core_modules/RWD_MODELS_FOR_TUNING/f12n5jti/best_model.pt

#aug_sigma_field_batch_pair_seed
#aug_sigma_field_batch_pair_seed
...
# batch_augmentations=default #convert batch to a pair and augment one of them 0.5p 
...
# #python train_rwd_model_bk.py experiment=$experiment ++data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$ltb trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=10 dloader.prefetch_factor=2 batch_size_all=1 #ckpt_path=$ckpt_path
# python train_rwd_model_bk.py callbacks=default_tloss experiment=$experiment data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$ltb trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=10 dloader.prefetch_factor=2 batch_size_all=1 #ckpt_path=$ckpt_path




batch_augmentations=aug_sigma_field_drop_none #dont drop any cases, augment good or orig mesh with prob 0.5 (two uniform sampel)
batch_augmentations=aug_sigma_field_batch_pair_seed #drop wp 0.5, aug wp 0.5
batch_augmentations=aug_sigma_field_batch_to_one #drop to one orig, and augment it
batch_augmentations=aug_sigma_field_batch_aug_all #apply to all
batch_augmentations=default #no augmentations
batch_augmentations=aug_sigma_field_batch_pair_seed #drop wp 0.5, aug wp 0.5, use w new resid architecture see what hap.
batch_augmentations=default #no augmentations
batch_augmentations=aug_sigma_field_orig_aug_drop_none #augment all the seeds in orig batch, and keep good seeed, but do not aug good seeed.
...
#ckpt_path=/home/krillman/Documents/eg3dredo/supp_plus_code/eg3d_rlhf_code/reward_model_training/reward_model_framework/core_modules/RWD_MODELS_FOR_TUNING/4xaxdave/best_model.pt
NWORK=10
...
# #python train_rwd_model_bk.py experiment=$experiment ++data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$ltb trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=10 dloader.prefetch_factor=2 batch_size_all=1 #ckpt_path=$ckpt_path
# python train_rwd_model_bk.py callbacks=default_tloss experiment=$experiment data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$vv trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=$NWORK dloader.prefetch_factor=2 batch_size_all=1 data.dset_dict.remove_middle_data=true data.dset_dict.row_size_to_keep=4 #ckpt_path=$ckpt_path


# python train_rwd_model_bk.py callbacks=default_tloss experiment=$experiment data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$vv trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=$NWORK dloader.prefetch_factor=2 batch_size_all=1 data.dset_dict.remove_middle_data=true data.dset_dict.row_size_to_keep=2 #ckpt_path=$ckpt_path


# python train_rwd_model_bk.py callbacks=default_tloss experiment=$experiment data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$vv trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=$NWORK dloader.prefetch_factor=2 batch_size_all=1 data.dset_dict.remove_middle_data=true data.dset_dict.row_size_to_keep=6 #ckpt_path=$ckpt_path


# python train_rwd_model_bk.py callbacks=default_tloss experiment=$experiment data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$vv trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=cos dloader.num_workers=$NWORK dloader.prefetch_factor=2 batch_size_all=1 data.dset_dict.remove_middle_data=true data.dset_dict.row_size_to_keep=4 #ckpt_path=$ckpt_path

python train_rwd_model_bk.py callbacks=default_tloss experiment=$experiment data.batch_augmentations=$batch_augmentations trainer.max_epochs=$epochs trainer.limit_val_batches=$ltb trainer.limit_train_batches=$vv trainer.limit_test_batches=$ltb prop_data=1.0 model.compile=false train_on=$train_on model.act_type=softplus dloader.num_workers=$NWORK dloader.prefetch_factor=2 batch_size_all=1 data.dset_dict.remove_middle_data=true data.dset_dict.row_size_to_keep=4 data.dset_dict.badseeds_models=[] ckpt_path=$ckpt_path
