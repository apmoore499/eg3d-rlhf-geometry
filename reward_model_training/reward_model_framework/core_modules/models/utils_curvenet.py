# https://github.com/tiangexiang/CurveNet/blob/main/core/models/utils_curvenet.py
"""
@Author: Yue Wang
...
class Walk(nn.Module):
    ...
    def __init__(self, in_channel, k, curve_num, curve_length):
        ...
        self.agent_mlp = nn.Sequential(nn.Conv2d(in_channel * 2, 1, kernel_size=1, bias=False), nn.BatchNorm2d(1))
        self.momentum_mlp = nn.Sequential(nn.Conv1d(in_channel * 2, 2, kernel_size=1, bias=False), nn.BatchNorm1d(2))
    ...
    def forward(self, xyz, x, adj, cur):
        ...
        for step in range(self.curve_length):
            ...
            if step != 0:
                ...
                d = self.crossover_suppression(cur_feature_cos - pre_feature_cos, pick_values_cos - cur_feature_cos.unsqueeze(-1), bn, self.curve_num, self.k)
...
def farthest_point_sample(xyz, npoint, device=torch.device("cuda")):
    """
    Input:
    ...
    device = xyz.device
    B, N, C = xyz.shape
    centroids = torch.zeros(B, npoint, dtype=torch.long, device=torch.device("cuda"))
    distance = torch.ones(B, N, device=torch.device("cuda")) * 1e10
    farthest = torch.randint(0, N, (B,), dtype=torch.long, device=torch.device("cuda")) * 0
    batch_indices = torch.arange(B, dtype=torch.long, device=torch.device("cuda"))
...
class Attention_block(nn.Module):
    ...
    def __init__(self, F_g, F_l, F_int):
        ...
        self.W_g = nn.Sequential(nn.Conv1d(F_g, F_int, kernel_size=1, stride=1, padding=0, bias=True), nn.BatchNorm1d(F_int))

        self.W_x = nn.Sequential(nn.Conv1d(F_l, F_int, kernel_size=1, stride=1, padding=0, bias=True), nn.BatchNorm1d(F_int))

        self.psi = nn.Sequential(nn.Conv1d(F_int, 1, kernel_size=1, stride=1, padding=0, bias=True), nn.BatchNorm1d(1), nn.Sigmoid())
...
class LPFA(nn.Module):
    def __init__(self, in_channel, out_channel, k, mlp_num=2, initial=False):
        ...
        if not initial:
            self.xyz2feature = nn.Sequential(nn.Conv2d(9, in_channel, kernel_size=1, bias=False), nn.BatchNorm2d(in_channel))

        self.mlp = []
        for _ in range(mlp_num):
            self.mlp.append(nn.Sequential(nn.Conv2d(in_channel, out_channel, 1, bias=False), nn.BatchNorm2d(out_channel), nn.LeakyReLU(0.2)))
    ...
    def group_feature(self, x, xyz, idx):
        ...
        point_feature = torch.cat((points, point_feature, point_feature - points), dim=3).permute(0, 3, 1, 2).contiguous()
...
class CIC(nn.Module):
    def __init__(self, npoint, radius, k, in_channels, output_channels, bottleneck_ratio=2, mlp_num=2, curve_config=None):
        ...
        self.conv1 = nn.Sequential(nn.Conv1d(in_channels, planes, kernel_size=1, bias=False), nn.BatchNorm1d(in_channels // bottleneck_ratio), nn.LeakyReLU(negative_slope=0.2, inplace=False))  # change to inpalce=False AM

        self.conv2 = nn.Sequential(nn.Conv1d(planes, output_channels, kernel_size=1, bias=False), nn.BatchNorm1d(output_channels))

        if in_channels != output_channels:
            self.shortcut = nn.Sequential(nn.Conv1d(in_channels, output_channels, kernel_size=1, bias=False), nn.BatchNorm1d(output_channels))

        self.relu = nn.LeakyReLU(negative_slope=0.2, inplace=False)  # change to inpalce=False AM
...
class CurveAggregation(nn.Module):
    def __init__(self, in_channel):
        ...
        self.convd = nn.Sequential(nn.Conv1d(mid_feature * 2, in_channel, kernel_size=1, bias=False), nn.BatchNorm1d(in_channel))
...
class MaskedMaxPool(nn.Module):
    ...
    def forward(self, xyz, features):
        ...
        sub_features = F.max_pool2d(neighborhood_features, kernel_size=[1, neighborhood_features.shape[3]])  # bs, c, n, 1
