...
def make2d(array, cols=None, dtype=None):
    ...
    The `cols' and `dtype' arguments can be omitted if the array is not empty.
    """
...
class PlyElement:
    ...
    def __init__(self, name, properties, count, comments=[]):
        ...
        The preferred methods of obtaining PlyElement instances are PlyData.read (to read from a file) and PlyElement.describe (to construct from a numpy array).
        """
        self._name = str(name)
        self._check_name()
        self._count = count
    ...
    def dtype(self, byte_order="="):
        """Return the numpy dtype of the in-memory representation of the data.

        (If there are no list properties, and the PLY format is binary, then this also accurately describes the on-disk representation of the element.)
        """
    ...
    def _parse_one(lines):
        """Consume one element definition.

        The unconsumed input is returned along with a PlyElement instance.
        """
        a = 0
        line = lines[a]
    ...
    def _read_txt(self, stream):
        ...
        The element may contain list properties.
        """
...
class PlyProperty:
    ...
    def _from_fields(self, fields):
        ...
        Raise StopIteration if the property could not be read.
        """
