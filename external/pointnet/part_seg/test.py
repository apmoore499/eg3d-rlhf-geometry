...
parser.add_argument(
    "--model_path",
    default="train_results/trained_models/epoch_190.ckpt",
...
oid2cpid = json.load(open(os.path.join(hdf5_data_dir, "overallid_to_catid_partid.json")))
...
color_map = json.load(open(color_map_file))

NUM_OBJ_CATS = 16
NUM_PART_CATS = 50

cpid2oid = json.load(open(os.path.join(hdf5_data_dir, "catid_partid_to_overallid.json")))
...
def output_color_point_cloud(data, seg, out_file):
    with open(out_file, "w") as f:
        ...
        for i in range(l):
            ...
            f.write(f"v {data[i][0]:f} {data[i][1]:f} {data[i][2]:f} {color[0]:f} {color[1]:f} {color[2]:f}\n")
...
def predict():
    ...
    with tf.Session(config=config) as sess:
        ...
        for cat_idx in range(NUM_OBJ_CATS):
            printout(
                flog,
                "\t " + objcats[cat_idx] + " Total Number: " + str(total_per_cat_seen[cat_idx]),
            )
            if total_per_cat_seen[cat_idx] > 0:
                printout(
                    flog,
                    "\t " + objcats[cat_idx] + " Accuracy: " + str(total_per_cat_acc[cat_idx] / total_per_cat_seen[cat_idx]),
                )
                printout(
                    flog,
                    "\t " + objcats[cat_idx] + " IoU: " + str(total_per_cat_iou[cat_idx] / total_per_cat_seen[cat_idx]),
                )
