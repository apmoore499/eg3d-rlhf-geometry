...
fin = open(all_obj_cats_file)
lines = [line.rstrip() for line in fin.readlines()]
all_obj_cats = [(line.split()[0], line.split()[1]) for line in lines]
fin.close()

all_cats = json.load(open(os.path.join(hdf5_data_dir, "overallid_to_catid_partid.json")))
NUM_CATEGORIES = 16
NUM_PART_CATS = len(all_cats)

print(f"#### Batch Size: {batch_size}")
print(f"#### Point Number: {point_num}")
print(f"#### Training using GPU: {FLAGS.gpu}")
...
print(f"### Training epoch: {TRAINING_EPOCHES}")
    ...
    with tf.Graph().as_default():
        ...
        def eval_one_epoch(epoch_num):
            ...
            (
                ...
                [
                    total_test_loss_sum_op,
                    label_test_acc_sum_op,
                    label_test_loss_sum_op,
                    seg_test_loss_sum_op,
                    seg_test_acc_sum_op,
                ],
                feed_dict={
            ...
            for cat_idx in range(NUM_CATEGORIES):
                if total_seen_per_cat[cat_idx] > 0:
                    ...
                    printout(
                        flog,
                        "\t\tCategory %s Label Accuracy: %f"
                        % (
                            all_obj_cats[cat_idx][0],
                            total_label_acc_per_cat[cat_idx] / total_seen_per_cat[cat_idx],
                        ),
                    )
                    printout(
                        flog,
                        "\t\tCategory %s Seg Accuracy: %f"
                        % (
                            all_obj_cats[cat_idx][0],
                            total_seg_acc_per_cat[cat_idx] / total_seen_per_cat[cat_idx],
                        ),
                    )
