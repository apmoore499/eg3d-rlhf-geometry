...
def collect_point_label(anno_path, out_filename, file_format="txt"):
    """Convert original dataset files to data_label file (each line is XYZRGBL). We aggregated all the points from each instance in the room.
...
def sample_data(data, num_sample):
    ...
    we want to keep num_samplexC of them. if N > num_sample, we will randomly keep num_sample of them. if N < num_sample, we will randomly duplicate samples.
    """
    N = data.shape[0]
    if N == num_sample:
        return data, range(N)
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride, random_sample, sample_num, sample_aug):
    ...
    data_batch, label_batch = room2blocks(data, label, num_point, block_size, stride, random_sample, sample_num, sample_aug)
...
def bbox_label_to_obj(input_filename, out_filename_prefix, easy_view=False):
    ...
    for i in range(bbox.shape[0]):
        ...
        fout_obj.write(f"v {x:f} {y:f} {z + c:f}\n")
        fout_obj.write(f"v {x:f} {y + b:f} {z + c:f}\n")
        fout_obj.write(f"v {x + a:f} {y + b:f} {z + c:f}\n")
        fout_obj.write(f"v {x + a:f} {y:f} {z + c:f}\n")
        fout_obj.write(f"v {x:f} {y:f} {z:f}\n")
        fout_obj.write(f"v {x:f} {y + b:f} {z:f}\n")
        fout_obj.write(f"v {x + a:f} {y + b:f} {z:f}\n")
        fout_obj.write(f"v {x + a:f} {y:f} {z:f}\n")
        ...
        fout_mtl.write(f"Kd {color[0]:f} {color[1]:f} {color[2]:f}\n")
        fout_mtl.write("\n")
        fout_obj.close()
        fout_mtl.close()
...
def collect_point_bounding_box(anno_path, out_filename, file_format):
    """Compute bounding boxes from each instance in original dataset files on one room. **We assume the bbox is aligned with XYZ coordinate.** Save both the point XYZRGB and the bounding box for the point's parent element.
