...
def launch_training_rlhf(c, desc, outdir, dry_run, hydra_cfg):
    ...
    if cfg.using_wandb:
        wandb.init(entity=cfg.wandb.entity, project=cfg.wandb.project, config=cfg_container,name=f"{c.run_dir}".split('/')[-1])
...
#@hydra.main(version_base=None, config_path="training/cfg_rlhf_tune_AM", config_name="rlhf_tune.yaml")



@hydra.main(version_base=None, config_path="training/rlhf_tune_configs", config_name="rlhf_extract_meshes_for_user_study.yaml")
def main_hydra(hydracfg : DictConfig) -> None:
    ...
    for k in orig_legacy_args.keys():
        renamed_orig[k.replace('-','_')]=orig_legacy_args[k]
    ...
    import wandb
    cfg=hydracfg
    using_wandb=cfg.using_wandb
    

    outdir=cfg.click_legacy_args.outdir
    
    
    #conditional populatin of fields
    if any([cfg.rlhf_tune_hpms.lambda_dmap_l1!=0.0,cfg.rlhf_tune_hpms.lambda_dmap_mse!=0.0]):       #,conf.lambda_lpips!=0.0]):
