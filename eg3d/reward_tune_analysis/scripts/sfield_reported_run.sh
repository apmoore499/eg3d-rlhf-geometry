...
# Fresh tune from the original EG3D FFHQ checkpoint (base resume, kimg 2048), 20 ticks.
...
LOGDIR="${EG3D_TRAINING_LOGDIR:-$REPO_ROOT/release_verification_outputs/eg3d_finetune_logs/sfield_reported}"
mkdir -p "$LOGDIR"
...
conda run --no-capture-output -n "$ENV" python train_rlhf.py \
  ...
  network_snapshot_ticks=20 export_first_images=true \
