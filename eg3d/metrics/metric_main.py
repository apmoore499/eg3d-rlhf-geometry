...
def report_metric(result_dict, run_dir=None, snapshot_pkl=None):
    ...
    if run_dir is not None and os.path.isdir(run_dir):
        with open(os.path.join(run_dir, f"metric-{metric}.jsonl"), "a") as f:
            f.write(jsonl_line + "\n")
...
# this will loop thru 1000 images for FID. Short time span use only in debugging.
@register_metric
def fid1k_partial(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=1000)
    return dict(fid1k_partial=fid)
