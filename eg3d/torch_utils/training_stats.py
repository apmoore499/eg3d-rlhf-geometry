...
"""Facilities for reporting and collecting training statistics across multiple processes and devices.

The interface is designed to minimize synchronization overhead as well as the amount of boilerplate in user code.
"""
...
def init_multiprocessing(rank, sync_device):
    r"""Initializes `torch_utils.training_stats` for collecting statistics across multiple processes.
...
def report(name, value):
    r"""Broadcasts the given set of scalars to all interested instances of `Collector`, across device and process boundaries.
...
def report0(name, value):
    r"""Broadcasts the given set of scalars by the first process (`rank = 0`), but ignores any scalars provided by the other processes.
...
class Collector:
    r"""Collects the scalars broadcasted by `report()` and `report0()` and computes their long-term averages (mean and standard deviation) over user-defined periods of time.
    ...
    def update(self):
        r"""Copies current values of the internal counters to the user-visible state and resets them for the next round.
...
def _sync(names):
    r"""Synchronize the global cumulative counters across devices and processes.

    Called internally by `Collector.update()`.
    """
    if len(names) == 0:
        return []
    global _sync_called
