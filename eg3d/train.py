...
def launch_training(c, desc, outdir, dry_run, hydra_cfg):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if c.num_gpus == 1:
            ...
            assert False, 'errro did not ipmlement multi gpu hydra'
...
def main(**kwargs):
    ...
    initialize(config_path=hydra_cfg_path)
    cfg = compose("config.yaml")#, overrides=["db=mysql", "db.user=${oc.env:USER}"])
    print(OmegaConf.to_yaml(cfg, resolve=True)

    # Launch.
    launch_training(c=c, hydra_cfg=cfg, desc=desc, outdir=opts.outdir, dry_run=opts.dry_run)
