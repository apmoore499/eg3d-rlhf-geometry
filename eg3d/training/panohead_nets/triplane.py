...
from training.panohead_nets.networks_stylegan2 import ToRGBLayer, SynthesisNetwork, MappingNetwork
from training.panohead_nets.networks_stylegan2 import Generator as StyleGAN2Backbone
from training.panohead_nets.volumetric_rendering.renderer import ImportanceRenderer
from training.panohead_nets.volumetric_rendering.ray_sampler import RaySampler
import dnnlib
...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False,
            ws_bcg=None, cache_backbone=False, use_cached_backbone=False,
            drop_super_res=False,       # AM: mirror of EG3D rlhf shim -- skip SR, return raw dict.
            depth_only=False,           # AM: mirror of EG3D rlhf shim -- only need the depth map.
            rgbs_thru_sigmoid=False,    # AM: accepted for signature-compat (unused here).
            **synthesis_kwargs):
        # `noise_mode` may arrive inside **synthesis_kwargs from the rlhf
        # loss (run_G). The PanoHead backbone / SR layers accept it, but
        # we also pop a clean copy so we can thread it explicitly and not
        # double-pass it to superresolution below.
        ...
        # depth_only: the rlhf depth-reg path only needs the depth map.
        # Mirror the EG3D rlhf shim and return early (same dict shape, plus
        # PanoHead's image_mask kept for downstream compatibility).
        if depth_only:
            weights_image = weights_samples.permute(0, 2, 1).reshape(N, 1, H, W)
            mask_image = weights_image * (1 + 2 * 0.001) - 0.001
            return {"image": -1, "image_raw": -1, "image_depth": depth_image, "image_mask": mask_image}
        ...
        # drop_super_res: the rlhf reward / depth path forwards only the raw
        # neural render. Skip the (expensive) SR stack and return raw dict.
        if drop_super_res:
            sr_image = torch.ones((rgb_image.shape[0], 1)).float().to(rgb_image.device)
...
from training.panohead_nets.networks_stylegan2 import FullyConnectedLayer
