...
def generate_planes():
    """Defines planes by the three vectors that form the "axes" of the plane.

    Should work with arbitrary number of planes and planes of arbitrary orientation.
    """
    return torch.tensor(
        [
            [[1, 0, 0], [0, 1, 0], [0, 0, 1]],
...
def project_onto_planes(planes, coordinates):
    """Does a projection of a 3D point onto a batch of 2D planes, returning 2D plane coordinates.

    Takes plane axes of shape n_planes, 3, 3 # Takes coordinates of shape N, M, 3 # returns projections of shape N*n_planes, M, 2
    """
    N, M, C = coordinates.shape
    n_planes, _, _ = planes.shape
    coordinates = coordinates.unsqueeze(1).expand(-1, n_planes, -1, -1).reshape(N * n_planes, M, 3)
    inv_planes = torch.linalg.inv(planes).to(coordinates.device).unsqueeze(0).expand(N, -1, -1, -1).reshape(N * n_planes, 3, 3)
...
class ImportanceRenderer(torch.nn.Module):
    ...
    def run_model(self, planes, decoder, sample_coordinates, sample_directions, options, rgbs_thru_sigmoid=True):
        ...
        out = decoder(sampled_features, sample_directions, sigmoid_final=rgbs_thru_sigmoid)
        if options.get("density_noise", 0) > 0:
            out["sigma"] += torch.randn_like(out["sigma"]) * options["density_noise"]

        # retval=dict(sampled_features=sampled_features,
    ...
    def sample_importance(self, z_vals, weights, N_importance):
        ...
        See NeRF importance sampling for more.
        """
        with torch.no_grad():
