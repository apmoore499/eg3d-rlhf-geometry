...
json_spec = dict(
    file_url="https://drive.google.com/uc?id=16N0RV4fHI6joBuKbQAoG34V_cQk7vxSA",
    file_path="ffhq-dataset-v2_10000.json",
    file_size=267793842,
    file_md5="425ae20f06a4da1d4dc0f46d40ba5fd6",
)
    ...
    for attempts_left in reversed(range(num_attempts)):
        ...
        try:
            ...
            if "file_size" in file_spec and data_size != file_spec["file_size"]:
                raise OSError("Incorrect file size", file_path)
            if "file_md5" in file_spec and data_md5.hexdigest() != file_spec["file_md5"]:
                raise OSError("Incorrect file MD5", file_path)
            if "pixel_size" in file_spec or "pixel_md5" in file_spec:
                with PIL.Image.open(tmp_path) as image:
                    if "pixel_size" in file_spec and list(image.size) != file_spec["pixel_size"]:
                        raise OSError("Incorrect pixel size", file_path)
                    if "pixel_md5" in file_spec and hashlib.md5(np.array(image)).hexdigest() != file_spec["pixel_md5"]:
                        raise OSError("Incorrect pixel MD5", file_path)
            break
    ...
    while True:
        ...
        print(
            ...
            % (
                ...
                files_done,
                files_total,
                f"{bytes_done / bytes_div:.2f}/{bytes_total / bytes_div:.2f} {bytes_unit}",
                f"{bandwidth / bandwidth_div:.2f} {bandwidth_unit}/s",
...
def run_cmdline(argv):
    ...
    parser.add_argument(
        "--start_index",
        help="start index for alignment (default: 0)",
        type=int,
        default=0,
        metavar="NUM",
    )
